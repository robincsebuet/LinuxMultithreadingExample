...
/*
 * Process a client request on a newly opened file descriptor inside a thread.
 */
void *
server_simple_thread_client_process(void * fd_pointer) {
	struct http_req *r;
	char *response;
	int len;
...
/*
 * takes a client request from que and process it for one of the thread of thread pool
 */
void *pool_bounded_server_process(void * queue_pointer) {

	struct http_req *r;
	char *response;
	int len;
	request_queue * accepted_req_queue;
	/*
	 * This code will be used to get the request and respond to
	 * it.  This should probably be in the worker
	 * threads/processes.
	 */
	accepted_req_queue = (request_queue *) queue_pointer;

	while (1) {
		int fd = get_request_from_queue(accepted_req_queue);
		printf("retreieved req from que %d \n", fd);
		//if (fd != 0) {
			r = newfd_create_req(fd);

			if (!r || !r->path) {
				close(fd);
				printf("Tiner chale kak\n");
			...
			respond_and_free_req(r, response, len);
			printf("serving one req finished \n");
		//}
	}
	return NULL;
}
